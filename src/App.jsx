...
const LEGAL_PAGES = {
  privacy: {
    ...
    note: {
      fa: 'ACCA EDU به‌عنوان مسئول داده، اطلاعاتی را که خودتان در فرم مشاوره وارد می‌کنید فقط برای بررسی درخواست، تماس از مسیر انتخابی و ارائه خدمات آموزشی پردازش می‌کند. اطلاعات به‌صورت الکترونیکی از همین فرم جمع‌آوری می‌شود و مبنای حقوقی، ضرورت پردازش مستقیم برای اقدام به درخواست مشاوره‌ای است که خودتان آغاز کرده‌اید و برقراری یا اجرای رابطه خدماتی مرتبط، در حدود ماده ۵/۲(c) قانون KVKK است. با ارسال فرم، نسخه اطلاعیه و زمان بازشدن فرم همراه درخواست ثبت می‌شود. اطلاعات در زیرساخت Supabase در منطقه اروپا ذخیره و برای اعلان داخلی به Telegram منتقل می‌شود؛ استفاده از این ارائه‌دهندگان می‌تواند شامل پردازش برون‌مرزی مشمول ماده ۹ KVKK باشد. Google Tag Manager و Meta Pixel فقط پس از انتخاب جداگانه شما فعال می‌شوند؛ نام، شماره تماس، سن، تحصیلات و معدل به این تگ‌ها ارسال نمی‌شود. می‌توانید انتخاب رهگیری را تغییر دهید و حقوق ماده ۱۱، از جمله درخواست دسترسی، اصلاح یا حذف، را از صفحه تماس با ما اعمال کنید.',
      en: 'ACCA EDU, as data controller, processes the details you submit in the consultation form only to review your request, contact you through your chosen channel, and provide education guidance. The details are collected electronically through this form. The legal basis is processing directly necessary to act on the consultation request you initiated and to establish or perform the related service relationship, within Article 5(2)(c) of Turkey’s KVKK. When you submit, the notice version and the time the form was opened are recorded with the request. Details are stored in Supabase infrastructure in the EU region and sent to Telegram for internal lead notification; using these providers may involve cross-border processing governed by Article 9 of the KVKK. Google Tag Manager and Meta Pixel activate only after your separate choice; your name, contact value, age, education, and GPA are not sent to these tags. You can change your tracking choice and exercise Article 11 rights, including access, correction, or deletion, through our contact page.',
    },
  },
  terms: {
